...
$page_title = 'Proposals'; $sp = 'proposals';
$topbar_crumb = 'Sales';
$topbar_action = '<a href="/pages/sales/document-edit.php?type=proposal" class="bf-topbar-new" style="text-decoration:none;"><i class="bi bi-plus-lg me-1"></i>New Proposal</a>';

$flash = $_SESSION['doc_flash'] ?? null; unset($_SESSION['doc_flash']);
$q  = trim($_GET['q'] ?? '');
$fs = trim($_GET['status'] ?? '');
$stats = doc_stats($uid, 'proposal');
$rows  = doc_all($uid, 'proposal', $q, $fs);
$cur = defined('CURRENCY') ? CURRENCY : 'KES';

$st = [
  'draft'    => ['Draft','grey'], 'sent'=>['Sent','navy'], 'open'=>['Open','blue'],
  'accepted' => ['Accepted','green'], 'declined'=>['Declined','red'], 'expired'=>['Expired','amber'],
];
?>
    ...
    <div class="bf-body" style="padding:24px 28px;">
      ...
      <div class="bf-dash-h">
        <div>
          ...
          <h1 class="bf-dash-title">Proposals</h1>
          <p class="bf-dash-sub">Create, send and track client proposals through to acceptance.</p>
        </div>
      </div>
      ...
      <div class="bf-kpis mb-3">
        <?php foreach ([
          ['Total proposals', number_format((int)$stats['cnt']),'bi-file-earmark-text','#1e3a5f','#eaf0f6'],
          ['Sent / open',     number_format((int)$stats['open_cnt']),'bi-send','#1e40af','#eff6ff'],
          ['Accepted',        number_format((int)$stats['won_cnt']),'bi-check2-circle','#166534','#f0fdf4'],
          ['Open value',      $cur.' '.number_format((float)$stats['pipeline_value']),'bi-cash-stack','#9a7d27','#fdf6e3'],
        ] as [$l,$v,$ic,$c,$bg]): ?>
      ...
      <form method="get" class="d-flex flex-wrap gap-2 mb-3">
        <div class="bf-topbar-search" style="max-width:320px;height:38px;"><i class="bi bi-search"></i><input name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Search proposals…"></div>
        <select name="status" class="form-select" onchange="this.form.submit()" style="width:auto;font-size:12.5px;border-color:var(--line);border-radius:9px;">
          ...
          <?php foreach (['draft'=>'Draft','sent'=>'Sent','open'=>'Open','accepted'=>'Accepted','declined'=>'Declined','expired'=>'Expired'] as $k=>$v): ?>
      ...
      <div class="bf-tbl-wrap">
        <div class="bf-tbl-head">
          <div style="width:110px;">Proposal</div>
          <div style="flex:1;">Client / Subject</div>
        ...
        <div style="padding:48px 20px;text-align:center;color:var(--ink-4);">
          <i class="bi bi-file-earmark-text" style="font-size:34px;opacity:.4;"></i>
          <p style="margin:12px 0 16px;font-size:13px;"><?= $q||$fs ? 'No proposals match your filter.' : 'No proposals yet. Create your first one.' ?></p>
          <a href="/pages/sales/document-edit.php?type=proposal" class="bf-topbar-new" style="text-decoration:none;"><i class="bi bi-plus-lg me-1"></i>New Proposal</a>
        </div>
        <?php endif; ?>
        ...
        <div class="bf-tbl-row" data-href="/pages/sales/document-view.php?id=<?= (int)$r['id'] ?>" style="cursor:pointer;">
          ...
          <div style="width:40px;text-align:right;" class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0" style="font-size:13px;border-radius:10px;">
              ...
              <li><form method="post" action="/pages/sales/document-action.php" style="margin:0;" onsubmit="return confirm('Create an estimate from this proposal?');"><input type="hidden" name="action" value="convert"><input type="hidden" name="id" value="<?= (int)$r['id'] ?>"><button class="dropdown-item" type="submit"><i class="bi bi-arrow-right-circle me-2 text-muted"></i>Convert to estimate</button></form></li>
